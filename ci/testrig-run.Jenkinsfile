def configs = [
              //["sail", "piccolo", "rv32izicsr_zifencei_xcheri", "rv32izicsr_zifencei_xcheri", ""]
                ["sail", "flute", "rv64imsafdczicsr_zifencei_xcheri", "rv64imsafdczicsr_zifencei_xcheri", "", "'.*'", "'^(atomic(64)?|memAmo(64)?|float(64)?|caprandom|capmisc|double(64)?|hpm|csr|all|capdecode|pte)\$'"]
        ...
        node('docker') {
          stage(name) {
            ...
            docker.image('ctsrd/testrig').inside {
              echo name
              sh "rm -f sail_coverage && /home/jenkins/TestRIG/utils/scripts/runTestRIG.py -a ${conf[0]} -b ${conf[1]} -r ${conf[2]} --verification-archstring ${conf[3]} ${conf[4]} -n 100 --test-include-regex ${conf[5]} --test-exclude-regex ${conf[6]}"
