def configs = [
              //["sail", "piccolo", "rv32izicsr_zifencei_xcheri",   "rv32izicsr_zifencei_xcheri", "", "'.*'", "'^(atomic(64)?|memAmo(64)?|float(64)?|double(64)?|hpm|csr|capdecode|pte|cloadtags)\$'"]
                ["sail", "flute",  "rv64imsafdczicsr_zifencei_xcheri", "rv64imsczifencei_xcheri", "", "'.*'", "'^(atomic(64)?|memAmo(64)?|float(64)?|double(64)?|hpm|csr|capdecode|pte|cloadtags)\$'", "Xnocloadtags"],
                ["sail", "toooba", "rv64imsafdczicsr_zifencei_xcheri", "rv64imsczifencei_xcheri", "", "'.*'", "'^(atomic(64)?|memAmo(64)?|float(64)?|double(64)?|hpm|csr|capdecode|pte|cloadtags)\$'", "Xnocloadtags"]
        ...
        node('docker') {
          stage(name) {
            ...
            docker.image('ctsrd/testrig').inside {
              ...
              def retval = sh returnStatus: true, script: "cd ${name} && mkdir failures && /home/jenkins/TestRIG/utils/scripts/runTestRIG.py -a ${conf[0]} -b ${conf[1]} -r ${conf[2]} --verification-archstring ${conf[3]} ${conf[4]} -n 100 --test-include-regex ${conf[5]} --test-exclude-regex ${conf[6]} -S failures/ --supported-features ${conf[7]}"
