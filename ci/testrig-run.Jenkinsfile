        ...
        node('docker') {
          stage(name) {
            ...
            docker.image('ctsrd/testrig').inside {
              echo name
              sh "rm -f sail_coverage && /home/jenkins/TestRIG/utils/scripts/runTestRIG.py -a ${conf[0]} -b ${conf[1]} -r ${conf[2]} --verification-archstring ${conf[3]} ${conf[4]} -n 10 --test-include-regex ${conf[5]} --test-exclude-regex ${conf[6]}"
