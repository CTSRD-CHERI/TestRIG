        ...
        node('docker') {
          stage(name) {
            ...
            docker.image('ctsrd/testrig').inside {
              ...
              def retval = sh returnStatus: true, script: "cd ${name} && mkdir failures && /home/jenkins/TestRIG/utils/scripts/runTestRIG.py -a ${conf[0]} -b ${conf[1]} -r ${conf[2]} --verification-archstring ${conf[3]} ${conf[4]} -n 100 --test-include-regex ${conf[5]} -S failures/"
