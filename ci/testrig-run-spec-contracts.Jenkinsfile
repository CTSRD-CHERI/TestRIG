        ...
        node('docker') {
          stage(name) {
            ...
            docker.image('ctsrd/testrig-pdr32').inside {
              ...
              def retval = sh returnStatus: true, script: "cd ${name} && mkdir failures && /home/jenkins/TestRIG/utils/scripts/runTestRIG.py -a ${conf[0]} --implementation-A-log implAlog.txt -b ${conf[1]} --implementation-B-log implBlog.txt -r ${conf[2]} --verification-archstring ${conf[3]} ${conf[4]} -n ${conf[7]} --test-include-regex ${conf[5]} --no-shrink --continue-on-fail --ignore-asserts -S failures/"
              sh "git -C /home/jenkins/TestRIG fetch --recurse-submodules && git -C /home/jenkins/TestRIG/ submodule sync --recursive && git -C /home/jenkins/TestRIG/ submodule update --init --recursive riscv-implementations/sail-cheri-riscv"
